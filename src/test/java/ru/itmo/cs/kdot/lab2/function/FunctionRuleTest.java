...
class FunctionRuleTest {
    ...
    void shouldNotAcceptOutside0And1(final FunctionRule function) {
        assertAll(
                () -> {
                    Throwable exception = assertThrows(ArithmeticException.class, () -> function.calculate(ONE, NEGATIVE_PRECISION));
                    assertEquals("Значение точности должно быть между 0 и 1 включительно", exception.getMessage());
                },
                () -> {
                    Throwable exception = assertThrows(ArithmeticException.class, () -> function.calculate(ONE, POSITIVE_PRECISION));
                    assertEquals("Значение точности должно быть между 0 и 1 включительно", exception.getMessage());
                }
        );
    }
